...
const reserve_post = async (req, res) => {
    ...
    var objForUpdate = {};
    objForUpdate.reservations = match.reservations.push({userId:data.userId,x:data.x,y:data.y});
...
const cancelReserve_put = async (req, res) => {
    ...
    if(found){
        ...
        var objForUpdate = {};
        objForUpdate.reservation = match.reservations;
        objForUpdate = { $set: objForUpdate };
        await Match.findOneAndUpdate({_id:data.id}, objForUpdate,function(err)
        {
            if(err)
